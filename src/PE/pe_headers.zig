...
const types = @import("pe_types.zig");
...
pub export fn parsePeFile(path: [*:0]const u8, out_info: *types.PeInfo) bool {
    ...
    const dos_bytes_read = file.readStreaming(io, &.{std.mem.asBytes(&dos_header)}) catch |err| {
        std.debug.panic("Failed to read DOS header: {s}; Error is: {}", .{ path, err });
        return true;
    };
    if (dos_bytes_read < @sizeOf(types.IMAGE_DOS_HEADER)) return true;
    if (dos_header.e_magic != types.IMAGE_DOS_SIGNATURE) return true;
    ...
    _ = file.readPositionalAll(io, std.mem.asBytes(&pe_signature), nt_offset) catch |err| {
        std.debug.panic("Failed to read NT signature: {s}; Error is: {}", .{ path, err });
        return true;
    };
    if (pe_signature != types.IMAGE_NT_SIGNATURE) return true;
    ...
    _ = file.readPositionalAll(io, std.mem.asBytes(&file_header), file_header_offset) catch |err| {
        std.debug.panic("Failed to read file header: {s}; Error is: {}", .{ path, err });
        return true;
    };
    ...
    _ = file.readPositionalAll(io, std.mem.asBytes(&opt_magic), opt_header_offset) catch |err| {
        std.debug.panic("Failed to read optional header magic: {s}; Error is: {}", .{ path, err });
        return true;
    };
    ...
    _ = file.readPositionalAll(io, std.mem.asBytes(&entry_point), opt_header_offset + 16) catch |err| {
        std.debug.panic("Failed to read entry point: {s}; Error is: {}", .{ path, err });
        return true;
    };
    ...
    var sections = allocator.alloc(types.PeInfo.SectionInfo, file_header.NumberOfSections) catch |err| {
        std.debug.panic("Failed to allocate memory for section headers: {s}; Error is: {}", .{ path, err });
        return true;
    };
    ...
    while (i < file_header.NumberOfSections) : (i += 1) {
        ...
        _ = file.readPositionalAll(io, std.mem.asBytes(&sec_header), current_sec_offset) catch |err| {
            std.debug.panic("Failed to read section header {d}: {s}; Error is: {}", .{ i, path, err });
            return true;
        };
